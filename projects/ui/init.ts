...
let rootEnvFile = fs.readFileSync("./../mcp-server/.env.template", "utf-8");
...
rootEnvFile = rootEnvFile.replace("<REPLACE_WITH_OPTIN_PLUGIN_ID>", optinPluginMintResults.appClient.appId.toString());
...
rootEnvFile = rootEnvFile.replace("<REPLACE_WITH_LISTING_FACTORY_ID>", listingFactoryMintResults.appClient.appId.toString());
...
rootEnvFile = rootEnvFile.replace("<REPLACE_WITH_MARKETPLACE_PLUGIN_ID>", marketPlacePluginMintResults.appClient.appId.toString());
const agentAccount = algosdk.generateAccount();

const dispenser = await algorand.account.dispenserFromEnvironment();

await algorand.account.ensureFunded(agentAccount.addr, dispenser, (1000000).algo());
...
fs.writeFileSync("../mcp-server/.env", rootEnvFile, "utf-8");
